...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (hasRemoteData) {
          console.log("Dados remotos encontrados. Sincronizando estado...");
          if (remServices) {
            const active = remServices.filter(s => s.status !== 'CONCLUÍDO').map(s => ({
              ...s, customerName: s.customer_name, budgetItems: s.budget_items, finishedDate: s.finished_date, imageUrl: s.image_url || ''
            }));
            const history = remServices.filter(s => s.status === 'CONCLUÍDO').map(s => ({
              ...s, customerName: s.customer_name, budgetItems: s.budget_items, finishedDate: s.finished_date, imageUrl: s.image_url || ''
            }));
            setServices(active);
          ...
          if (remSettings) {
            ...
            setFinishedCountToday(remSettings.last_reset_date === today ? remSettings.finished_count_today : 0);
          }
        } else {
          // 2. Migração local para Cloud (Caso o Supabase esteja vazio)
          console.log("Nenhum dado remoto. Verificando LocalStorage para migração...");
          const savedData = localStorage.getItem(STORAGE_KEY);
          if (savedData) {
            const p = JSON.parse(savedData);

            // Set state localmente primeiro
  ...
  // Salvar configs (Cloud + Local backup) - Só após carregar
  useEffect(() => {
  ...
  }, [workshopInfo, nextServiceNumber, finishedCountToday, isLoaded, session?.user?.id]);
